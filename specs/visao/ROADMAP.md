...
| ⚠️ Build do Read the Docs **nunca exercitado com uv** | A tradução dos jobs de Poetry para uv em `.readthedocs.yaml` não foi testada no ambiente real do RTD, e o `pipeline.yml` não cobre esse build. Se estiver quebrado, o sintoma é a documentação publicada parar de atualizar **em silêncio** | Nenhum. Verificar em readthedocs.org se o build da `main` ficou verde depois da mudança 001 |
